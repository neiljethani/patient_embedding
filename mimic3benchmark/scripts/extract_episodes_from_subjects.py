...
from mimic3benchmark.preprocessing import read_itemid_to_variable_map, map_itemids_to_variables, read_variable_ranges, clean_events, clean_events_remove_outliers 
...
subjects_root_iter = tqdm.tqdm(enumerate(os.listdir(args.subjects_root_path)),
                               desc="Patients Processed:",
                               total=len(os.listdir(args.subjects_root_path)))

for subject_num, subject_dir in subjects_root_iter:
    ...
    events = clean_events_remove_outliers(events, ranges)
    if events.shape[0] == 0:
