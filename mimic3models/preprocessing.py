...
class DiscretizerContinuous:
    def __init__(self, timestep=0.8, store_masks=True, impute_strategy='zero', start_time='zero',
                 config_path=os.path.join(os.path.dirname(__file__), 'resources/discretizer_continuous_config.json')):
    ...
    def transform(self, X, header=None, end=None):
        ...
        def write(data, bin_id, channel, value):
            channel_id = self._channel_to_id[channel]
            if self._is_categorical_channel[channel]:
                category_value = self._possible_values[channel][value]
                data[bin_id, channel_id] = float(category_value)
            else:
        ...
        return (data, self._id_to_channel)
