...
class ClassificationDataset(data.Dataset):
    ...
    def get_seq_length(self):
        return int(self.reader._period_length)
...
class HourlyClassificationDataset(data.Dataset):
    def __init__(self, embed_method, reader, discretizer, normalizer=None, return_name=False):
        self.reader = reader
        self.discretizer = discretizer
        self.normalizer = normalizer
    ...
    def __getitem__(self, index):
        ...
        X = self.discretizer.transform(X, end=t)[0][-int(p):]
        if self.normalizer is not None:
            X = self.normalizer.transform(X)
